'''
AUTHOR: Dan Grissom
Student 1: Roberto Rodriguez
Student 2: Johana Chazaro Cortes
...
def SimulatedAnnealing():
    ...
    print ("The initial solution has " + str(cost) + " pairs of queens in attacking position.")
    ...
    # Costs will be the val from Fitness(currentBoardState, True)
    s = initialBoardArray  # intitial state NOTE: We may need to put this in a set. there is a chance we get the same state multiple times
    e = cost  # cost of current state. 
    sBest= s
    eBest = e
    k = 0
    kMax = 20000
    ...
    tMax = 1
    t = tMax
    while k < kMax and e > eGoal:
        print("Iteration: " + str(k))
        t = tMax - t*0.09
        sNew = GenerateRandomNeighborState(s)
        ...
        if(eNew < e):
            ...
            if (e < eBest):
                sBest = sNew
                eBest = eNew
        elif random.random() < e**((eBest-eNew)/t):
            s = sNew
            e = eNew
        k = k+1
    Fitness(sBest, True)
    ...
    for i in range(0, len(boardArray)-1):
        for j in range(i+1, len(boardArray)):            
            if (boardArray[i] == boardArray[j]): # Check horizontal
                ...
                if verbose: print ("Attacking Horizontal: (" + str(i+1) + ", " + str(boardArray[i]) + ")<-->(" + str(j+1) + ", " + str(boardArray[j]) + ")")
            elif (abs(i-j) == abs(boardArray[i] - boardArray[j])): # Check diagonal
                attackingQueenPairs = attackingQueenPairs + 1
                if verbose: print ("Attacking Diagonal: (" + str(i+1) + ", " + str(boardArray[i]) + ")<-->(" + str(j+1) + ", " + str(boardArray[j]) + ")")
